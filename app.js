  ...
  container.innerHTML = CATEGORIES.map(category => {
    ...
    return `
      <button 
        ...
        class="${activeClass} px-5 py-2 rounded-full font-medium transition-all border border-leaf-200">
        ${category}
      </button>
    `;
  }).join("");
...
function renderProducts() {
  ...
  grid.innerHTML = filtered.map(product => `
    <div 
      class="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all hover:-translate-y-1 overflow-hidden border border-leaf-100 cursor-pointer group"
      ...
      <div class="p-5">
        <span class="text-xs font-bold text-leaf-600 uppercase tracking-wide">
          ${product.category}
        </span>

        <h3 class="text-lg font-bold text-gray-800 mt-1 mb-2">
          ${product.name}
        </h3>

        <p class="text-leaf-700 font-bold text-xl mb-4">
          ${formatPrice(product.price)}
        </p>

        <button 
          onclick="event.stopPropagation(); showProduct(${product.id})"
          class="w-full bg-leaf-100 hover:bg-leaf-200 text-leaf-800 py-2 rounded-xl font-semibold transition-colors">
          Ver detalhes
        </button>
      </div>
    </div>
  ...
  cart.forEach(item => {
    ...
    row.innerHTML = `
      ...
      <div class="flex items-center bg-earth-100 rounded-full overflow-hidden">
        <button onclick="changeCartQuantity(${item.id}, -1)" class="px-3 py-1">−</button>
...
function renderCheckoutSummary() {
  ...
  container.innerHTML = cart.map(item => `
    <div class="flex justify-between items-center py-2 border-b border-leaf-100">
